...
export default async function () {
  ...
  await Promise.all([
    fs.promises.copyFile(path.join(PROJ_ROOT, 'stubs', 'config.mjs'), path.join(CONFIG_DIR, 'config.mjs')),
    fs.promises.copyFile(path.join(PROJ_ROOT, 'stubs', 'ascii'), path.join(CONFIG_DIR, 'ascii')),
    getAndSaveToken(),
  ])
}
